#!/usr/bin/env bash
# Tier A factory build: async from git source.
# Upstream publishes from build/ after `make` (not npm run build). See Makefile release target.
...
echo "[build.entrypoint] Installing dependencies for make build"
npm install --include=dev --ignore-scripts

echo "[build.entrypoint] Running make (staging build/ for npm publish layout)"
make build-es build-modules build-bundle build-dist build-config

echo "[build.entrypoint] Packing from build/ (matches upstream npm publish)"
rm -f "${main_tgz}"
cd build
...
assert_tgz_has_member "${main_tgz}" package/dist/async.js
assert_tgz_has_member "${main_tgz}" package/dist/async.mjs
