#!/usr/bin/env bash
# Tier A factory build: node-fetch from git source via npm pack.
# Handles version mismatch by patching package.json before packing.
set -euo pipefail
...
# Check if package.json version matches expected version
current_version="$(jq -r .version package.json)"
if [[ "${current_version}" != "${VERSION}" ]]; then
    echo "[build.entrypoint] Patching package.json version from ${current_version} to ${VERSION}"
