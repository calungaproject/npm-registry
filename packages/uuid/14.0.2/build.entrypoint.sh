#!/usr/bin/env bash
# Tier A factory build: uuid from git source with TypeScript build step.
...
echo "[build.entrypoint] Installing dependencies (with devDependencies for build)"
npm install --include=dev --ignore-scripts

echo "[build.entrypoint] Building TypeScript sources"
npm run build

echo "[build.entrypoint] Packing from built checkout"
rm -f "${main_tgz}"
