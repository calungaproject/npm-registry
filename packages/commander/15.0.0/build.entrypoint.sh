#!/usr/bin/env bash
# Tier A factory build: commander from git source via npm pack (local checkout only).
...
expected_sha="ba6d13ddb4243e5913367734f8c159089ffe7834"
[[ "${actual_sha}" == "${expected_sha}" ]] || {
    echo "Source commit ${actual_sha} != expected ${expected_sha} for tag ${SOURCE_REF}" >&2
    exit 1
}
