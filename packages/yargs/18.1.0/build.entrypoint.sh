#!/usr/bin/env bash
# Tier A factory build: yargs from git source with build step.
...
echo "[build.entrypoint] Installing dependencies (NODE_ENV=production requires --include=dev for build tools)"
...
assert_tgz_has_member "${main_tgz}" package/build/index.cjs
