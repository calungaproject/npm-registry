#!/usr/bin/env bash
# Tier A factory build: yargs from git source with npm install + npm run build.
...
echo "[build.entrypoint] Installing dependencies with --include=dev (factory sets NODE_ENV=production)"
npm install --include=dev --ignore-scripts

echo "[build.entrypoint] Running build step"
npm run compile

echo "[build.entrypoint] Packing from built tree (npm pack)"
rm -f "${main_tgz}"
...
assert_tgz_has_member "${main_tgz}" package/index.cjs
