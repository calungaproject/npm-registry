...
echo "[build.entrypoint] Installing dependencies (npm install --include=dev --ignore-scripts)"
npm install --include=dev --ignore-scripts

echo "[build.entrypoint] Running build (npm run prepare)"
npm run prepare

echo "[build.entrypoint] Packing from built source (npm pack)"
rm -f "${main_tgz}"
