...
echo "[verify.smoke] Installing tarball for require test"
npm install --ignore-scripts --no-audit --no-fund --prefix "${tmpdir}" "${MAIN_PATH}"

echo "[verify.smoke] Testing module require"
node -e "
const yargs = require('yargs');
if (typeof yargs !== 'function') {
    console.error('yargs module did not export a function');
    process.exit(1);
}
console.log('yargs module loaded successfully');
" || {
