...
for member in package/package.json package/index.mjs package/build/index.cjs; do
...
node -e 'const yargs = require(process.argv[1]); const argv = yargs(["--foo", "bar"]).argv; if (argv.foo !== "bar") process.exit(1); console.log("yargs API test passed")' "${tmpdir}/node_modules/yargs"
