#!/usr/bin/env bash
# Post-build smoke checks for yargs Tier A output.
set -euo pipefail
...
echo "[verify.smoke] Testing module API with require"
node -e 'const yargs = require(process.argv[1]); const argv = yargs(["--help"]).argv; if (!yargs) process.exit(1); console.log("yargs loaded successfully")' "${tmpdir}/node_modules/yargs"

echo "[verify.smoke] Testing basic functionality"
node "${tmpdir}/node_modules/yargs/index.mjs" --version 2>&1 || true
