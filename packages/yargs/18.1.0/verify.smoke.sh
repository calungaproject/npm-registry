...
# Assert essential package members exist
for member in package/package.json package/index.mjs package/index.cjs; do
...
# Test syntax of built files
tmpdir="$(mktemp -d)"
trap 'rm -rf "${tmpdir}"' EXIT

tar -xOf "${MAIN_PATH}" package/index.mjs > "${tmpdir}/index.mjs"
tar -xOf "${MAIN_PATH}" package/index.cjs > "${tmpdir}/index.cjs"

node --check "${tmpdir}/index.mjs" || {
    echo "index.mjs syntax check failed" >&2
    exit 1
}

node --check "${tmpdir}/index.cjs" || {
    echo "index.cjs syntax check failed" >&2
    exit 1
}

echo "[verify.smoke] Testing module require"
npm install --ignore-scripts --no-audit --no-fund --prefix "${tmpdir}" "${MAIN_PATH}"
node -e "const yargs = require('yargs'); if (!yargs) throw new Error('yargs require failed')" || {
    echo "Module require test failed" >&2
    exit 1
}
