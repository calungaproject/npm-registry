#!/usr/bin/env bash
# Post-build smoke checks for chalk Tier A output.
set -euo pipefail
...
node -e 'import("file://" + process.argv[1] + "/node_modules/chalk/source/index.js").then(m => { const chalk = m.default; console.log(chalk.blue("test")); })' "${tmpdir}"
