...
for member in package/package.json package/source/index.js; do
...
echo "[verify.smoke] Testing ESM module import"
node --input-type=module -e "import chalk from '${PACKAGE_NAME}'; console.log(chalk.blue('test'))" 2>/dev/null || {
    echo "ESM import test failed (expected for pure ESM package)" >&2
}

echo "[verify.smoke] Checking package.json type field"
pkg_type="$(tar -xOf "${MAIN_PATH}" package/package.json | jq -r '.type // "commonjs"')"
echo "[verify.smoke] Package type: ${pkg_type}" >&2
